...
protocol DeclarationCollecting {
    ...
    /// When capable, will collect any immediate child declarations, such as `Structure`, `Class`, `Enumeration`, `Function`, etc from the node being
    /// explored.
    ///
    ...
    /// **Note:** A valid `SparrowSourceLocationConverter` instance is required for accurate location resolving, this is taken from the `context` on
    /// the instance.
