...
class FunctionParameterSemanticsResolver: ParameterNodeSemanticsResolving {
    ...
    private func resolveEntityType() -> EntityType {
        guard let type = node.type else { return .empty }
        return EntityType.parseType(type)
    }
    ...
    private func resolveIsInout() -> Bool {
        node.type?.tokens(viewMode: .fixedUp).contains(where: { $0.tokenKind == .inoutKeyword }) ?? false
    }
}
